...
class Week_Cal {
	...
	var $next_prev_url	= 'index.php?module=home';
	...
	function generateWeek($week = '', $year = '', $month = '', $data = array())
{
    ...
    $adjusted_date = $this->adjust_date($week, $month, $year);
   ...
   $start_day = ( ! isset($start_days[$this->start_day])) ? 0 : $start_days[$this->start_day];
    ...
    $local_date =  strtotime($year.'W'.$week); //strtotime('monday this week');  //mktime(12, 0, 0, $month, date('j'), $year);
    ...
    if ($this->show_next_prev == TRUE)
    {
       ...
       // $this->next_prev_url = preg_replace("/(.+?)\/*$/", "\\1/",  $this->next_prev_url);
        ...
        $adjusted_date = $this->adjust_date($week - 1, $month, $year);
        $out .= str_replace('{previous_url}', $this->next_prev_url.'&year='.$adjusted_date['year'].'&week='.$adjusted_date['week'], $this->temp['heading_previous_cell']);
        $out .= "\n";
    }
    ...
    $this->temp['heading_title_cell'] = str_replace('{heading}', '(Week # '.$week.') '.$this->get_month_name($month)."&nbsp;".$year, $this->temp['heading_title_cell']);
    ...
    {
        $adjusted_date = $this->adjust_date($week + 1, $month, $year);
        $out .= str_replace('{next_url}', $this->next_prev_url.'&year='.$adjusted_date['year'].'&week='.$adjusted_date['week'], $this->temp['heading_next_cell']);
    }
...
*/
	 ...
	 * @return	array
	 */
	function adjust_date($week, $month, $year)
	{
		$date = array();
	...
	function get_day_names($day_type = '')
	{
		...
		foreach ($day_names as $val)
		{			
			$days[] = ($val === FALSE) ? ucfirst($val) : $val;
		}
