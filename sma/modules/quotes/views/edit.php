...
<?php if($message) { echo "<div class=\"alert alert-error\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;</button>" . $message . "</div>"; } ?>
	...
	<?php $attrib = array('class' => 'form-horizontal', 'id' => 'addSale_form'); echo form_open("module=quotes&view=edit&id=".$id, $attrib); ?>
...
<label class="control-label"><?php echo $this->lang->line("invoice_items"); ?></label>
  ...
  <thead>
      ...
      <?php if(PRODUCT_SERIAL) { echo '<th class="span2">'.$this->lang->line("serial_no").'</th>'; } ?>
  ...
  <?php $r = 1; foreach($inv_products as $prod){ 
			 ...
			 if(PRODUCT_SERIAL) { echo '<td><input class="span2 tran2" name="serial'. $r .'" type="text" value="'.$prod->serial_no.'"></td>'; } 
			if(DISCOUNT_OPTION == 2) { echo '<td><select class="span2 tran" data-placeholder="Select..." name="discount'.$r.'" id="discount'. $r .'">';
				foreach($discounts as $discount) {
					echo "<option value=" . $discount->id;
					if($discount->id == $prod->discount_id) { echo ' selected="selected"'; }
					echo ">" . $discount->name . "</option>";
				}
			echo '</select></td>'; } 
			
			if(TAX1) { echo '<td><select class="span2 tran" data-placeholder="Select..." name="tax_rate'.$r.'" id="tax_rate'.$r.'">';
				foreach($tax_rates as $tax) {
					echo "<option value=" . $tax->id;
					if($tax->id == $prod->tax_rate_id) { echo ' selected="selected"'; }
					echo ">" . $tax->name . "</option>";
				}
			echo '</select></td>'; }
...
<div class="control-group">
<label class="control-label" for="note"><?php echo $this->lang->line("on_quote_note"); ?></label>
  <div class="controls">
