...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	private function sync() 
	{
		...
		if (!$this->db->field_exists('quantity', 'products'))
		{
		  	$this->load->dbforge();
			$field = array(
			...
			$this->db->update('settings', array('version' => '2.2'), array('setting_id' => 1));
		} 
			
		$q = $this->db->get('products');
		if($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
				$wQty = $this->db->select("COALESCE(sum(warehouses_products.quantity), 0) as totalQuantity", FALSE)->get_where('warehouses_products', array('product_id' => $row->id))->result_array();
				$this->db->update('products', array('quantity' => $wQty[0]['totalQuantity']), array('id' => $row->id));
			}
	...
	public function get_total_results()
    {
		if(VERSION < '2.2.1') {
			if($this->sync()) {
				$this->db->update('settings', array('version' => '2.2.1'), array('setting_id' => 1));
				$this->session->set_flashdata('success_message', 'Version successfully updated to 2.2.1');
	...
	public function getEvents() 
	{
		...
		if(CAL_OPT) {
			$q = $this->db->get_where('calendar', array('user_id' => USER_ID)); 
		} else {
