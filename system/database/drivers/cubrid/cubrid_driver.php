...
class CI_DB_cubrid_driver extends CI_DB {
	...
	// Default CUBRID Broker port. Will be used unless employee
	...
	function db_connect()
	{
		// If no port is defined by the employee, use the default value
		if ($this->port == '')
		{
		...
		if ($conn)
		{
			// Check if a employee wants to run queries in dry, i.e. run the
	...
	function _escape_identifiers($item)
	{
		...
		foreach ($this->_reserved_identifiers as $id)
		{
			if (strpos($item, '.'.$id) !== FALSE)
			{
				...
				// remove duplicates if the employee already included the escape
